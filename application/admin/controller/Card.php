...
class Card extends Base
{
    ...
    public function index()
    {
        ...
        if($param['export'] =='1'){
            ...
            foreach($res['list'] as  $k=>$v){
                echo $v['card_no'] . "," . $v['card_money'] . "," . date('Y-m-d H:i:s',$v['card_add_time']) . "\n";
            }
    ...
    public function info()
    {
        if (Request()->isPost()) {
            ...
            $res = model('Card')->saveAllData(intval($param['num']),intval($param['money']),intval($param['point']),$param['role_no'],$param['role_pwd'],intval($param['vip']),intval($param['tian']));
            if($res['code']>1){
