...
class Card extends Base {
    ...
    public function saveAllData($num,$money,$point,$role_no,$role_pwd,$vip,$tian)
    {
        $data=[];
        $t = time();
        for($i=1;$i<=$num;$i++){
            ...
            $data[$card_no] = ['card_no'=>$card_no,'card_money'=>$money,'card_points'=>$point,'card_add_time'=>$t,'card_vip'=>$vip,'card_tian'=>$tian];
        }
    ...
    public function useData($card_no,$user_info)
    {
		...
		if($info['card_vip']!==""){
			$time=$user_info['user_end_time'];
			if($time <= time()){
				$time=time();
			}
			$time=$time+$info['card_tian']*86400;
			Db::query("UPDATE  `mac_user` SET  `group_id` =  '".$info['card_vip']."',`user_end_time` =  '".$time."' WHERE  `mac_user`.`user_id` ='".$user_info['user_id']."';");
		}else{
			$where2=[];
