...
class User extends Base
{
    ...
    public function index()
    {
		...
		$num=$user['user_points'];
		$site=$GLOBALS['config']['site'];

		if($user['user_end_time']<time()){
			$user['user_end_time']=time();
		}

		if($num>=$site['site_point_3']){
			$time=$user['user_end_time']+$site['site_time_3']*86400;
			$schema = Db::query("UPDATE `mac_user` SET  `group_id` =  '3',
			`user_end_time` =  '".$time."',
			`user_points` =  user_points-".$site['site_point_3']." WHERE  `user_id` ='".$GLOBALS['user']['user_id']."';");
		}

		else if($num>=$site['site_point_2']){
			$time=$user['user_end_time']+$site['site_time_2']*86400;
			$schema = Db::query("UPDATE `mac_user` SET  `group_id` =  '3',
			`user_end_time` =  '".$time."',
			`user_points` =  user_points-".$site['site_point_2']." WHERE  `user_id` ='".$GLOBALS['user']['user_id']."';");
		}

		else if($num>=$site['site_point_1']){
			$time=$user['user_end_time']+$site['site_time_1']*86400;
			$schema = Db::query("UPDATE `mac_user` SET  `group_id` =  '3',
			`user_end_time` =  '".$time."',
			`user_points` =  user_points-".$site['site_point_1']." WHERE  `user_id` ='".$GLOBALS['user']['user_id']."';");
		}

		else if($num>=$site['site_point_0']){
			$time=$user['user_end_time']+$site['site_time_0']*86400;
			$schema = Db::query("UPDATE `mac_user` SET  `group_id` =  '3',
			`user_end_time` =  '".$time."',
			`user_points` =  user_points-".$site['site_point_0']." WHERE  `user_id` ='".$GLOBALS['user']['user_id']."';");
		}
